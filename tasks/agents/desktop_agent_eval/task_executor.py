...
from .desktop_agent_metrics import DesktopAgentMetricsCalculator
from tasks.agents.web_agent_eval.error_detection_utils import is_server_error
...
class ServiceNowRequestResponseLogger:
    ...
    def setup_logger():
        ...
        # Use the local logs directory within the desktop_agent_eval task
        current_dir = os.path.dirname(os.path.abspath(__file__))
        log_dir = os.path.join(current_dir, 'logs')
        os.makedirs(log_dir, exist_ok=True)

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        log_file = os.path.join(log_dir, f'desktop_agent_requests_{timestamp}.json')
    ...
    def apply(lambda_node_dict: dict, state: SygraState) -> SygraState:
        ...
        try:
            ...
            if image_path:
                ...
                logger.info(
                    f"SeedDataProcessor: Successfully injected base64 form into state, so it can be rendered in prompt")
            else:
...
class FetchNextActionPreProcessor(NodePreProcessor):
    ...
    def inline_evaluation(state: SygraState, prev_retry_data: dict, retry_number: int) -> dict:
        ...
        if golden_response:
            ...
            predicted_tool = evaluation_result.get('predicted_tool', 'unknown')
            golden_tool = evaluation_result.get('golden_tool', 'unknown')
            ...
            logger.info(f"FetchNextActionPostProcessor: Retry {retry_number} evaluation - "
                        ...
                        f"Predicted: {predicted_tool}, Expected: {golden_tool}")
    ...
    def retry_failure_and_chat_injection(state: SygraState, curr_retry: int, retry_prompt_injection: str) -> SygraState:
        ...
        if retry_prompt_injection == 'yes':
            ...
            if not tool_correct:
                failure_hint = f"Your previous tool call '{predicted_tool}' was incorrect. Do NOT use this tool again in the retry, even with different arguments. \nIf you believe the previous tool might still be right, dismiss that belief and proceed with a different tool call."
            else:
                failure_hint = f"Your previous tool call '{predicted_tool}' was correct, but had incorrect parameters. Use the same tool again but give the correct parameters in this retry."
            ...
            logger.info(
                f"FetchNextActionPreProcessor: retry_prompt_injection is '{retry_prompt_injection}', keeping original prompt")

        return state

    @staticmethod
    def apply(state: SygraState) -> SygraState:
        """Process Claude's response and advance to next step."""
        logger.info("FetchNextActionPreProcessor: Setting up for LLM Call")
        logger.debug(f"FetchNextActionPreProcessor: State keys: {list(state.keys())}")
        state['__chat_history__'] = state['chat_history']
        curr_retry = state.get('curr_retries', 0)

        state["image_width_50"] = state["image_width"] * 0.5
        state["image_height_50"] = state["image_height"] * 0.5
        state["image_width_25"] = state["image_width"] * 0.25
        state["image_height_30"] = state["image_height"] * 0.3

        logger.debug(f"FetchNextActionPreProcessor: Current retry: {curr_retry}")

        current_tool_result = state.get("current_tool_result", {})
        logger.debug(f"FetchNextActionPreProcessor: current_tool_result type: {type(current_tool_result)}")
        logger.debug(f"FetchNextActionPreProcessor: current_tool_result keys: {current_tool_result.keys() if isinstance(current_tool_result, dict) else 'Not a dict'}")
        
        # this tool call details not required, as it is fetched from chat history last assistant message
        # state["current_tool_call_id"] = current_tool_result.get("tool_call_id")
        # state["current_name"] = current_tool_result.get("name")
        # state["current_content"] = "success"
        if len(current_tool_result.get("content", [])) == 1 and current_tool_result.get("content", [])[0].get("image"):
            ...
            logger.debug(f"FetchNextActionPreProcessor: Extracted screenshot with format: {img_fmt}")
        else:
            logger.debug(f"FetchNextActionPreProcessor: No screenshot found in current_tool_result")
        ...
        logger.debug(f"FetchNextActionPreProcessor: Retry config: {retry_config}")

        # Check if retry chat injection is required from graph_properties
        retry_injection_required = retry_config.get("required", "no")
        logger.debug(f"FetchNextActionPreProcessor: Retry injection required: {retry_injection_required}")
        ...
        if retry_injection_required == "yes":
            ...
            state = FetchNextActionPreProcessor.retry_failure_and_chat_injection(state, curr_retry,
                                                                                 retry_prompt_injection)
            logger.info(
                f"FetchNextActionPreProcessor: Retry chat injection applied (prompt_injection={retry_prompt_injection})")
        else:
...
class FetchNextActionPostProcessor(NodePostProcessorWithState):
    """
    Post-processor that handles Claude's response and advances to next step.
    """
    ...
    def apply(resp: SygraMessage, state: SygraState) -> SygraState:
        ...
        logger.debug(f"FetchNextActionPostProcessor: Response type: {type(resp.message)}")
        logger.debug(f"FetchNextActionPostProcessor: Response has content attr: {hasattr(resp.message, 'content')}")

        # Log the raw response for debugging
        retry_number = state["curr_retries"]
        logger.debug(f"FetchNextActionPostProcessor: Processing retry number: {retry_number}")
        ServiceNowRequestResponseLogger.log_response(resp.message, retry_number)

        # Extract response content
        llm_response = resp.message
        logger.debug(f"FetchNextActionPostProcessor: LLM response type: {type(llm_response)}")

        # Handle the response based on type
        if hasattr(llm_response, 'content'):
            content = llm_response.content
            logger.debug(f"FetchNextActionPostProcessor: Extracted content from .content attribute")
        else:
            content = str(llm_response)
            logger.debug(f"FetchNextActionPostProcessor: Converted response to string")
        
        logger.debug(f"FetchNextActionPostProcessor: Content length: {len(str(content))}")
        ...
        # # load json response
        # try:
        #     logger.debug(f"FetchNextActionPostProcessor: Attempting to parse content as JSON")
        #     resp_dict = json.loads(content)
        #     response_text = resp_dict.get("resp_text", "")
        #     tool_calls = resp_dict.get("tool_calls", [])
        #     logger.debug(f"FetchNextActionPostProcessor: Parsed JSON - text_length={len(response_text)}, tool_calls_count={len(tool_calls)}")
        #     logger.debug(f"FetchNextActionPostProcessor: Response dict keys: {resp_dict.keys()}")
        # except Exception as e:
        #     logger.warning(f"FetchNextActionPostProcessor: Failed to parse as JSON: {e}")
        #     response_text = str(content) if content else ""
        #     tool_calls = []
        #     logger.debug(f"FetchNextActionPostProcessor: Using raw content as text response")
        ...
        logger.debug(f"FetchNextActionPostProcessor: Created response_dict with keys: {response_dict.keys()}")

        if "model_responses" not in state:
            state["model_responses"] = {}
            logger.debug(f"FetchNextActionPostProcessor: Initialized model_responses in state")
        ...
        logger.debug(f"FetchNextActionPostProcessor: Total model_responses stored: {len(state['model_responses'])}")
        ...
        logger.info(f"FetchNextActionPostProcessor: ===== POST-PROCESSOR COMPLETE =====")
        ...
        try:
            if not data:
                ...
                return [{
                    "evaluation_summary": {
                        "total_records": 0,
                        "timestamp": timestamp,
                        "evaluation_type": "desktop_agent_metrics",
                        "status": "no_data"
                    },
                    "results": {
                        "overall": {},
            ...
            return [{
                ...
                "results": {
                    "overall": {},
                    "mission": {},
                    "summary": {"total_records": len(data) if data else 0, "processed_records": 0,
                                "skipped_records": len(data) if data else 0}
                }
            }]