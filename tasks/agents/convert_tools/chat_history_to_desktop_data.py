...
parser.add_argument("-t", "--task", type=str, default="agents/desktop_agent_eval", help="task name.",
                    required=False)
...
def get_golden_data_for_event(record: dict) -> dict:
    """
    Extract golden response from record using the golden_response_extractor module.
    
    Args:
        record (dict): Complete record with chat_history, bbox, typedValue, event_type
    
    Returns:
        dict: Golden response with tool_name, tool_input, bbox, etc.
...
# Removed get_scroll_direction - now handled by golden_response_extractor module
...
for r in range(rows):
    ...
    if current_scneario_id != prev_scenario_id:
        turn = 1
    else:
        turn += 1
    ...
    navigational_direction = dataf[r]["navigational_directions"]
    image_width = dataf[r]["image_resolution"]["display_width"]
    image_height = dataf[r]["image_resolution"]["display_height"]

    # JSON Loads chat history - handle None/empty cases (first record of each mission)
    try:
        if chat_history is None or chat_history == "" or chat_history == "null" or len(chat_history) < 4:
            raise ValueError("Empty or incomplete chat_history")
        modified_chat_history = json.loads(chat_history) if isinstance(chat_history, str) else chat_history
    ...
    # Use the golden response extractor module to get golden response
    # This handles all tool types including typing, scroll, mouse_move, clicks, etc.
    try:
        golden_response = get_golden_data_for_event(dataf[r])
        tool_name = golden_response.get('tool_name', 'unknown')
        print(f"✓ Extracted golden response for tool: {tool_name}")
    except (ValueError, KeyError, IndexError, TypeError) as e:
        print(f"✗ Error extracting golden response for record {r}: {e}")
        empty_chat_history_count += 1
        continue
