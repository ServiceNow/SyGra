"""
Customer Service Agent with Ultra-Realistic Audio V3 - Task Executor

This module uses REAL background noise from the MUSAN dataset (HuggingFace).
...
# ============== Audio Synthesis Lambda with MUSAN ==============
...
class SynthesizeWithMusanNoise:
    ...
    async def apply(lambda_node_dict: dict, state: SygraState):
        ...
        musan_audio = state.get("musan->audio")
        noise_source = "musan"
        ...
        try:
            ...
            # Load and process MUSAN noise if available
            noise_samples = None
            noise_sr = 24000
            ...
            for turn in turns:
                ...
                # For user turns, mix with MUSAN noise
                if role == "user" and noise_samples and turn_segments:
                    ...
                    # Decode speech
                    speech_samples, speech_sr = AudioProcessor.decode_wav_bytes(turn_audio)
                    ...
                    # Random crop or loop noise to match speech length
                    if len(processed_noise) > len(speech_samples):
                        processed_noise = AudioProcessor.random_crop(processed_noise, len(speech_samples))
                    else:
                    ...
                    # Apply subtle gain drift for realism
                    ...
                    # Convert back to WAV
                    turn_audio_with_noise = AudioProcessor.float_to_wav_bytes(mixed, speech_sr)
...
class CustomerServiceAudioOutputGenerator(BaseOutputGenerator):
    ...
    def build_personality_metadata(data: Any, state: SygraState) -> dict:
        ...
        return {
            ...
            "noise_level": state.get("noise_level", "moderate"),
