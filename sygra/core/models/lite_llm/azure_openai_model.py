...
class CustomAzureOpenAI(BaseCustomModel):
    ...
    async def _generate_text(
        ...
        """
        Generate text using Azure OpenAI Chat or Completions API.
        ...
        try:
            ...
            logger.warning(
                f"[{self.name()}] Azure OpenAI API request exceeded rate limit: {e.message}"
            )
            resp_text = f"{constants.ERROR_PREFIX} Azure OpenAI API request exceeded rate limit: {e.message}"
            ret_code = getattr(e, "status_code", 429)
        except BadRequestError as e:
            logger.error(f"[{self.name()}] Azure OpenAI API bad request: {e.message}")
            resp_text = f"{constants.ERROR_PREFIX} Azure OpenAI API bad request: {e.message}"
            ret_code = getattr(e, "status_code", 400)
        except APIError as e:
            logger.error(f"[{self.name()}] Azure OpenAI API error: {e.message}")
            resp_text = f"{constants.ERROR_PREFIX} Azure OpenAI API error: {e.message}"
            ret_code = getattr(e, "status_code", 500)
        except Exception as e:
            logger.error(f"[{self.name()}] Azure OpenAI text generation failed: {e}")
            resp_text = f"{constants.ERROR_PREFIX} Azure OpenAI text generation failed: {e}"
    ...
    async def _generate_speech(
        ...
        """
        Generate speech from text using Azure OpenAI TTS API.
        ...
        try:
            ...
            logger.warning(f"[{self.name()}] Azure OpenAI TTS API request exceeded rate limit: {e}")
            resp_text = (
                f"{constants.ERROR_PREFIX} Azure OpenAI TTS API request exceeded rate limit: {e}"
            )
            ret_code = getattr(e, "status_code", 429)
        except BadRequestError as e:
            logger.error(f"[{self.name()}] Azure OpenAI TTS bad request: {e.message}")
            resp_text = f"{constants.ERROR_PREFIX} Azure OpenAI TTS bad request: {e.message}"
            ret_code = getattr(e, "status_code", 400)
        except APIError as e:
            logger.error(f"[{self.name()}] Azure OpenAI TTS request failed with error: {e.message}")
            resp_text = (
                f"{constants.ERROR_PREFIX} Azure OpenAI TTS request failed with error: {e.message}"
            )
            ret_code = getattr(e, "status_code", 500)
        except Exception as e:
            logger.error(f"[{self.name()}] Azure OpenAI TTS request failed with error: {e}")
            resp_text = f"{constants.ERROR_PREFIX} Azure OpenAI TTS request failed with error: {e}"
            rcode = self._get_status_from_body(e)
            ret_code = rcode if rcode else 999
    ...
    async def _generate_image(
        ...
        """
        Generate or edit images using Azure OpenAI Image API.
        ...
        try:
            ...
            logger.warning(
                f"[{self.name()}] Azure OpenAI Image API request exceeded rate limit: {e.message}"
            )
            resp_text = f"{constants.ERROR_PREFIX} Azure OpenAI Image API request exceeded rate limit: {e.message}"
            ret_code = getattr(e, "status_code", 429)
        except BadRequestError as e:
            logger.error(f"[{self.name()}] Azure OpenAI Image API bad request: {e.message}")
            resp_text = f"{constants.ERROR_PREFIX} Azure OpenAI Image API bad request: {e.message}"
            ret_code = getattr(e, "status_code", 400)
        except APIError as e:
            logger.error(f"[{self.name()}] Azure OpenAI Image API error: {e.message}")
            resp_text = f"{constants.ERROR_PREFIX} Azure OpenAI Image API error: {e.message}"
            ret_code = getattr(e, "status_code", 500)
        except Exception as e:
