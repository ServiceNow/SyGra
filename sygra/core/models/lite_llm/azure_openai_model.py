...
class CustomAzureOpenAI(BaseCustomModel):
    ...
    async def _generate_transcription(
        ...
        try:
            ...
            logger.warning(f"[{self.name()}] Azure OpenAI Transcription API request exceeded rate limit: {e}")
            resp_text = f"{constants.ERROR_PREFIX} Rate limit exceeded: {e}"
            ret_code = 429
        except openai.APIError as e:
            logger.error(f"[{self.name()}] Azure OpenAI Transcription API error: {e}")
            resp_text = f"{constants.ERROR_PREFIX} API error: {e}"
            ret_code = getattr(e, "status_code", 500)
        except Exception as x:
