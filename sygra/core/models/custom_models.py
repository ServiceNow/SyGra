...
class BaseCustomModel(ABC):
    ...
    def get_chat_formatted_text(
        ...
        if self.tokenizer:
            ...
            chat_formatted_text = chat_format_object
...
class CustomClaudeProxy(BaseCustomModel):
        def __init__(self, model_config: dict[str, Any]) -> None:
            ...
            self.model_config = model_config
            if model_config.get("auth_token"):
        ...
        def _convert_openai_tool_call_to_model_format(self, messages: list[dict[Any, Any]])->list:
            """
                ...
                if m.get("role") == "assistant":
                    ...
                    if tool_calls and len(tool_calls) > 0:
                        for t in tool_calls:
                            ...
                            m["content"].append({"toolUse": {"name": name, "toolUseId": tool_call_id, "input":args}})
                            assisstant_tool_call_results.append({"toolResult": {"toolUseId": tool_call_id, "status": "success", "content": [{"success": "true"}]}})
                    else:
                        assisstant_tool_call_results = []
                    ...
                    logger.debug("Claude Proxy Model: Found tool result as tool role, drop it for Claude model in proxy.")
        ...
        def _convert_tool_use_to_openai(self, tool_use:dict) -> dict:
            if tool_use and len(tool_use.keys()) > 0:
                ...
                return {"id": tool_use_id, "type": "function", "function": {"name": name, "arguments": json.dumps(input)}}
            else:
                return {}
            ...
            try:
                ...
                for message in mmchat_messages:
                    if message.get("role") == "system" and len(message.get("content", [])) > 0:
                        system_msg = message.get("content")[0].get("text", "")
                    ...
                    if len(json_resp.get("output", {}).get("message", {}).get("content", [])) > 0:
                        for msg in json_resp.get("output", {}).get("message", {}).get("content", []):
                            if msg.get("text"):
                                ...
                                raise NotImplementedError(f"Not implemented for this type of message: {str(msg)}")
...
class CustomGeminiProxy(BaseCustomModel):
        ...
        if kwargs.get("tools"):
            ...
            for t in tools:
                ...
                if isinstance(t, dict):
                    ...
                    gemini_tool = copy(t.get('function'))
    ...
    def _convert_functions_to_parts(selfself, contents):
        func_parts = []
        map_id_to_funcname = {}
        for c in contents:
            for k, v in c.items():
                if k == "function":
                    try:
                        func_parts.append(
                            {"functionCall": {"name": v.get("name"), "args": json.loads(v.get("arguments"))}})
                    except:
    ...
    def _convert_openai_tool_call_to_model_format(self, messages: list[str]) -> list:
        """
        ...
        for msg in messages:
            ...
            if role == "user":
                ...
                assistant_tool_call_results.append({"functionResponse": {"name": funcname,
    ...
    def _convert_tool_use_to_openai(self, tool_use: dict, toolcall_id: str) -> dict:
        if tool_use and len(tool_use.keys()) > 0:
            ...
            return {"id": toolcall_id, "type": "function", "function": {"name": name, "arguments": json.dumps(input)}}
        else:
            return {}
                ...
                for part in parts:
                    for k, v in part.items():
                        ...
                        if k == "text":
                            ...
                            logger.warning(f"Gemini Custom Model: Only supports text and functionCall. Unknown type received: {k}")
                            continue
