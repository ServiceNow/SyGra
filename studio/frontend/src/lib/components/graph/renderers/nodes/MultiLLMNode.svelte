...
>
	<div class="space-y-1.5">
		{#if modelCount > 0}
			{#each modelEntries.slice(0, 3) as [label, config]}
				<div class="flex items-center justify-between text-xs">
					<div class="flex items-center gap-1.5">
						<Bot size={12} class="text-info" />
						<span class="font-medium text-text-secondary truncate max-w-[70px]" title={label}>
							{label}
						</span>
					</div>
			...
			{/if}
		{:else}
			<div class="text-xs text-text-muted italic text-center py-1">
