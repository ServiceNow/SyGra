...
from core.graph.backend_factory import BackendFactory
from core.graph.langgraph.langgraph_factory import LangGraphFactory
from data_mapper.helper import JSONEncoder
...
def get_models_used(task: str):
    ...
    grasp_yaml = load_yaml_file(f"tasks/{task}/graph_config.yaml")
...
def get_graph_factory(backend: str) -> BackendFactory:
...
def get_graph_properties(task_name: str = None) -> dict:
    ...
    if not task:
        raise ValueError("Current task name is not initialized.")

    formatted_task_name = "/".join(task.split("."))
    path = f"tasks/{formatted_task_name}/graph_config.yaml"
...
backend_factory = get_graph_factory(constants.BACKEND)
